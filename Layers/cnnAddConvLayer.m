...
if cnn.to.useGPU==1
    CLayer.W=0.1*randn(FDim(1), FDim(2), cnn.Layers{cnn.LNum}.FNum, FNum, 'single');
    if comp=='c'
        CLayer.W=CLayer.W+0.1*1j*randn(FDim(1), FDim(2), cnn.Layers{cnn.LNum}.FNum, FNum, 'single');
    end
    CLayer.B=zeros(FNum, FNum, 'single');
else

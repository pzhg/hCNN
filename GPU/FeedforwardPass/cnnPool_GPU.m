...
switch PLayer.poolMethod
    ...
    case 'max'
        ...
        parfor imageNum=1:numImages
            for featureNum=1:numFilters
                temp=im2col(gather(convolvedFeatures(:, :, featureNum, imageNum)), [PLayer.poolDim(1), PLayer.poolDim(2)], 'distinct');
                [m, i]=max(temp, [], 1);
                poolLocation(1, :, featureNum, imageNum)=i;
                pooledFeatures(:, :, featureNum, imageNum)=reshape(gpuArray(m), PLayer.OutDim(1), PLayer.OutDim(2)); 
                % clear temp, m;
            end
        end    

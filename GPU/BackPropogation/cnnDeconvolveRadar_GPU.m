...
DeltaKa=zeros(RLayer.OutDim(1), RLayer.OutDim(2), numImages, 'single');
DeltaKr=zeros(RLayer.OutDim(1), RLayer.OutDim(2), numImages, 'single');
...
parfor i_im=1:numImages
    r_image=rot90(gpuArray(single(real(images(:, :, i_im)))), 2);
    i_image=rot90(gpuArray(single(imag(images(:, :, i_im)))), 2);
    DeltaKa(:, :, i_im)=gather(real(Delta(:, :, :, i_im)).*(conv2(r_image, FKaSin, 'valid')+conv2(i_image, FKaCos, 'valid'))+imag(Delta(:, :, :, i_im)).*(conv2(i_image, FKaSin, 'valid')-conv2(r_image, FKaCos, 'valid')));
    DeltaKr(:, :, i_im)=gather(real(Delta(:, :, :, i_im)).*(-conv2(r_image, FKrSin, 'valid')-conv2(i_image, FKrCos, 'valid'))+imag(Delta(:, :, :, i_im)).*(-conv2(i_image, FKrSin, 'valid')+conv2(r_image, FKrCos, 'valid')));  
end